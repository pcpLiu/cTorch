...
void test_acos(CTH_BACKEND backend, CTH_TENSOR_DATA_TYPE data_type, float min,
  ...
  if (data_type == CTH_TENSOR_DATA_TYPE_FLOAT_16 ||
    ...
    _ele_wise_equal_unary(float, EXPECT_FLOAT_EQ, acos);
  } else if (data_type == CTH_TENSOR_DATA_TYPE_FLOAT_64) {
    _ele_wise_equal_unary(double, EXPECT_DOUBLE_EQ, acos);
  } else if (data_type == CTH_TENSOR_DATA_TYPE_INT_16) {
    _ele_wise_equal_unary(int16_t, EXPECT_EQ, acos);
  } else if (data_type == CTH_TENSOR_DATA_TYPE_INT_32) {
    _ele_wise_equal_unary(int32_t, EXPECT_EQ, acos);
  } else if (data_type == CTH_TENSOR_DATA_TYPE_INT_64) {
    _ele_wise_equal_unary(int64_t, EXPECT_EQ, acos);
  } else if (data_type == CTH_TENSOR_DATA_TYPE_UINT_8) {
    _ele_wise_equal_unary(uint8_t, EXPECT_EQ, acos);
  }
}
