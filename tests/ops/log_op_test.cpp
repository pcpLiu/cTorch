...
void test_log(CTH_BACKEND backend, CTH_TENSOR_DATA_TYPE data_type, float min,
  ...
  CTHNode *op_node = create_dummy_op_node_unary(CTH_OP_ID_log, dims, n_dim,
                                                data_type, min, max);
...
TEST(cTorchLogOpTest, testFloat32MKL) {
  test_log(CTH_BACKEND_MKL, CTH_TENSOR_DATA_TYPE_FLOAT_32, 0.01, 20.0);
}
...
TEST(cTorchLogOpTest, testFloat64MKL) {
  test_log(CTH_BACKEND_MKL, CTH_TENSOR_DATA_TYPE_FLOAT_64, 0.01, 20.0);
}

#ifdef BACKEND_APPLE
TEST(cTorchLogOpTest, testFloat32Apple) {
  test_log(CTH_BACKEND_APPLE, CTH_TENSOR_DATA_TYPE_FLOAT_32, 0.01, 20.0);
}

TEST(cTorchLogOpTest, testFloat64Apple) {
  test_log(CTH_BACKEND_APPLE, CTH_TENSOR_DATA_TYPE_FLOAT_64, 0.01, 20.0);
}
#endif

#ifdef BACKEND_CUDA
TEST(cTorchLogOpTest, testFloat32CUDA) {
  test_log(CTH_BACKEND_CUDA, CTH_TENSOR_DATA_TYPE_FLOAT_32, 0.01, 20.0);
}

TEST(cTorchLogOpTest, testFloat64CUDA) {
  test_log(CTH_BACKEND_CUDA, CTH_TENSOR_DATA_TYPE_FLOAT_64, 0.01, 20.0);
}
#endif
