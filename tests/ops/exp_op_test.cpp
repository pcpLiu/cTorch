...
void test_exp(CTH_BACKEND backend, CTH_TENSOR_DATA_TYPE data_type, float min,
  ...
  CTHNode *op_node = create_dummy_op_node_unary(CTH_OP_ID_exp, dims, n_dim,
                                                data_type, min, max);
...
#ifdef BACKEND_CUDA
TEST(cTorchExpOpTest, testFloat32CUDA) {
  test_exp(CTH_BACKEND_CUDA, CTH_TENSOR_DATA_TYPE_FLOAT_32, -1.0, 1.0);
}

TEST(cTorchExpOpTest, testFloat64CUDA) {
  test_exp(CTH_BACKEND_CUDA, CTH_TENSOR_DATA_TYPE_FLOAT_64, -1.0, 1.0);
}
#endif
