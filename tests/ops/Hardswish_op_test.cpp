...
torch::Tensor
_Hardswish_pytorch(torch::Tensor &pytorch_in_tensor, CTHOperator *op) {
  auto ret = at::hardswish(pytorch_in_tensor);
  return std::move(ret);
}
...
void test_hardswish(
  ...
  if (data_type == CTH_TENSOR_DATA_TYPE_FLOAT_16 ||
    ...
    _ele_wise_equal_nn_op_pytorch(
        op, float, EXPECT_EQ_PRECISION, 1e-3, _Hardswish_pytorch);
  } else if (data_type == CTH_TENSOR_DATA_TYPE_FLOAT_64) {
    _ele_wise_equal_nn_op_pytorch(
        op, double, EXPECT_EQ_PRECISION, 1e-3, _Hardswish_pytorch);
  }
}

TEST(cTorchHardswishOpTest, testFloat32Default) {
  test_hardswish(
      CTH_BACKEND_DEFAULT, CTH_TENSOR_DATA_TYPE_FLOAT_32, -10.0, 10.0);
}

TEST(cTorchHardswishOpTest, testFloat64Default) {
  test_hardswish(
