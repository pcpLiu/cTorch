...
torch::Tensor
_ReLU6_pytorch(torch::Tensor &pytorch_in_tensor, CTHOperator *op) {
  auto m = torch::nn::ReLU6(torch::nn::ReLU6Options());
...
void test_reLU6_1d(
  ...
  if (data_type == CTH_TENSOR_DATA_TYPE_FLOAT_16 ||
    ...
    _ele_wise_equal_nn_op_pytorch(
        op, float, EXPECT_EQ_PRECISION, 1e-3, _ReLU6_pytorch);
  } else if (data_type == CTH_TENSOR_DATA_TYPE_FLOAT_64) {
    _ele_wise_equal_nn_op_pytorch(
        op, double, EXPECT_EQ_PRECISION, 1e-3, _ReLU6_pytorch);
  }
}

TEST(cTorchReLU6OpTest, testFloat32Default) {
  test_reLU6_1d(
      CTH_BACKEND_DEFAULT, CTH_TENSOR_DATA_TYPE_FLOAT_32, -10.0, 10.0);
}

TEST(cTorchReLU6OpTest, testFloat64Default) {
  test_reLU6_1d(
