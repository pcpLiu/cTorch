...
torch::Tensor
_LogSigmoid_pytorch(torch::Tensor &pytorch_in_tensor, CTHOperator *op) {
  auto m = torch::nn::LogSigmoid();

  auto ret = m(pytorch_in_tensor);
  return std::move(ret);
}

void test_logSigmoid_1d(
  ...
  if (backend == CTH_BACKEND_DEFAULT) {
    op_LogSigmoid_cpu(op);
  }

  if (data_type == CTH_TENSOR_DATA_TYPE_FLOAT_16 ||
      data_type == CTH_TENSOR_DATA_TYPE_FLOAT_32) {
    _ele_wise_equal_nn_op_pytorch(
        op, float, EXPECT_EQ_PRECISION, 1e-3, _LogSigmoid_pytorch);
  } else if (data_type == CTH_TENSOR_DATA_TYPE_FLOAT_64) {
    _ele_wise_equal_nn_op_pytorch(
        op, double, EXPECT_EQ_PRECISION, 1e-3, _LogSigmoid_pytorch);
  }
}

TEST(cTorchLogSigmoidOpTest, testFloat32Default) {
  test_logSigmoid_1d(
      CTH_BACKEND_DEFAULT, CTH_TENSOR_DATA_TYPE_FLOAT_32, -10.0, 10.0);
}

TEST(cTorchLogSigmoidOpTest, testFloat64Default) {
  test_logSigmoid_1d(
