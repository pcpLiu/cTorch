...
/**
 * @brief Returns a new tensor with the hyperbolic tangent of the elements of
 * input.
 *
 * Inputs & outputs:
 *    - # of input: 1
 ...
 */
void op_tanh_cpu(CTorchOperator *op) {
  ...
  _cpu_1d_map_elewise_unary(
      in->values, out->values, in->meta_info->data_type, N, tanh);
}
