...
#define _cth_reLU_kernel(op, data_type)                                        \
  do {                                                                         \
...
/**
 * @brief Applies the element-wise function reLU
 *
 ...
 *    - 0: output tensor
 */
void op_ReLU_cpu(CTHOperator *op) {
  FORCE_OP_INPUT_OUTPUT_TENSOR_NUM(op, 1, 1);

  CTHTensor *input = cth_array_at(CTHTensor)(op->in_bound_tensors, 0);
  CTH_TENSOR_DATA_TYPE data_type = input->meta_info->data_type;
  _cpu_generic_compute(op, _cth_reLU_kernel, data_type);
}
