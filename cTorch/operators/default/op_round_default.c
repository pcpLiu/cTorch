...
/**
 * @brief Returns a new tensor with each of the elements of input rounded to the
 * closest integer.
 *
 * Inputs & outputs:
 *    - # of input: 1
 ...
 */
void op_round_cpu(CTorchOperator *op) {
  ...
  _cpu_1d_map_elewise_unary(
      in->values, out->values, in->meta_info->data_type, N, round);
}
