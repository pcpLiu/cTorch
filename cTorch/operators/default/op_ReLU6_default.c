...
#define _cth_reLU6_kernel(op, data_type)                                       \
  do {                                                                         \
    ...
    for (cth_tensor_dim_t i = 0; i < N; i++) {                                 \
      if (input_ptr[i] >= 6) {                                                 \
        output_ptr[i] = (data_type)6.0;                                        \
      } else if (input_ptr[i] >= 0) {                                          \
...
/**
 * @brief Applies the element-wise function reLU6
 *
 ...
 *    - 0: output tensor
 */
void op_ReLU6_cpu(CTHOperator *op) {
  FORCE_OP_INPUT_OUTPUT_TENSOR_NUM(op, 1, 1);

  CTHTensor *input = cth_array_at(CTHTensor)(op->in_bound_tensors, 0);
  CTH_TENSOR_DATA_TYPE data_type = input->meta_info->data_type;
  _cpu_generic_compute(op, _cth_reLU6_kernel, data_type);
}
